...
class AuthController {
  ...
  static async getConnect(request, response) {
    const AuthHeader = request.header('Authorization').split(' ');
      ...
      // <email>:<password> will be split
    const [email, password] = Buffer.from(header[1], 'utf8').toString('base64').split(':');
    ...
    const specialTok = { token };
    return response.status(200).send(specialTok);
  }
